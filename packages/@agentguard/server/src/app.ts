...
export interface AgentGuardServerConfig {
  guard?: AgentGuard;
...
export function createApp(config: AgentGuardServerConfig = {}): Express {
  const guard = config.guard ?? new AgentGuard({ taintEnabled: true, promptShield: { enabled: true } });
